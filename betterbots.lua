...
local function safe_say(unit, line, important, skip_forced)
    ...
    if snd and snd.say then
        safe_call(snd.say, snd, tostring(line), important, skip_forced)
    end
end
...
local function shield_blocks(attacker, target_head_pos)
    if not (attacker and target_head_pos) then
        return false
    end
    if not (MASK and MASK.enemy_shield_check) then
        return false
    end
...
BB._enemy_flag_cache = BB._enemy_flag_cache or {}

local function _infer_flags_from_name(name)
    local f = {}
    if not name then
        return f
    end
    name = tostring(name):lower()
    if name:find("turret") then f.turret = true end
    if name:find("shield") then f.shield = true end
    if name:find("tank") or name:find("dozer") then f.dozer = true end
    if name:find("taser") then f.taser = true end
    if name:find("spooc") or name:find("cloaker") then f.cloaker = true end
    if name:find("medic") then f.medic = true end
    if name:find("sniper") then f.sniper = true end
    if name:find("phalanx") or name:find("captain") then f.captain = true end
    return f
end

local function _merge_flags(dst, src)
...
function BB.classify_enemy(unit, att_obj)
    ...
    local now = game_time()
    local cache = BB._enemy_flag_cache[u_key]
    ...
    local flags = {
        ...
        shield = false, dozer = false, taser = false, cloaker = false,
        medic = false, sniper = false, captain = false, special = false
    }

    if att_obj then
        if att_obj.is_shield then flags.shield = true end
        if att_obj.is_very_dangerous then flags.special = true end
    end

    if base and base.has_tag then
        if base:has_tag("shield") then flags.shield = true end
        if base:has_tag("tank") then flags.dozer = true end
        if base:has_tag("taser") then flags.taser = true end
        if base:has_tag("spooc") then flags.cloaker = true end
        if base:has_tag("sniper") then flags.sniper = true end
        if base:has_tag("medic") then flags.medic = true end
        if base:has_tag("phalanx") then flags.captain = true end
    end
    ...
    if tweak_name then
        ...
        if direct then
            _merge_flags(flags, direct)
        else
            _merge_flags(flags, _infer_flags_from_name(tweak_name))
        end
    end

    if char_tweak and char_tweak.tags then
        for tag, on in pairs(char_tweak.tags) do
            if on then
                if tag == "shield" then flags.shield = true
                elseif tag == "tank" then flags.dozer = true
                elseif tag == "taser" then flags.taser = true
                elseif tag == "spooc" then flags.cloaker = true
                elseif tag == "sniper" then flags.sniper = true
                elseif tag == "medic" then flags.medic = true
                elseif tag == "phalanx" then flags.captain = true
                end
            end
        end
    end
    ...
    if flags.shield or flags.dozer or flags.taser or flags.cloaker or flags.sniper or flags.medic or flags.captain then
        flags.special = true
    end
...
local function is_turret_unit(unit)
    return unit and BB.classify_enemy(unit).turret or false
end

local function is_shield_unit(unit, att_obj)
    return BB.classify_enemy(unit, att_obj).shield or false
end

local function is_special_unit(unit, att_obj)
    return BB.classify_enemy(unit, att_obj).special or false
end

local function is_dozer_unit(unit)
    return BB.classify_enemy(unit).dozer or false
end

local function is_sniper_unit(unit)
    return BB.classify_enemy(unit).sniper or false
end

local function is_taser_unit(unit)
    return BB.classify_enemy(unit).taser or false
end

local function is_cloaker_unit(unit)
    return BB.classify_enemy(unit).cloaker or false
end

local function is_medic_unit(unit)
    return BB.classify_enemy(unit).medic or false
end
    ...
    if TeamAILogicIdle then
        ...
        local function calculate_threat_value(bot_unit, target_data, data)
            ...
            if not flags.turret then
                local hr2 = get_unit_health_ratio(target_unit)
                if hr2 < 0.3 then
...
if RequiredScript == "lib/units/player_team/logics/teamailogicassault" then
    if TeamAILogicAssault then
                ...
                if attention_info.identified and (attention_info.verified or attention_info.nearly_visible) then
                    ...
                    if alive(att_unit) then
                        ...
                        if reaction >= REACT_COMBAT then
                            ...
                            local is_turret = flags.turret
                            local shieldish = flags.shield
                            local is_special = flags.special or is_turret
        ...
        function TeamAILogicAssault.mark_enemy(data, criminal, to_mark, play_sound, play_action)
            ...
            local is_special_enemy = is_special_unit(to_mark)
    ...
    if CopDamage then
        ...
        Hooks:PostHook(CopDamage, "die", "BB_CopDamage_die_post_cleanup", function(self, attack_data, ...)
            ...
            if u_key then
                ...
                if BB._enemy_flag_cache then
                    BB._enemy_flag_cache[u_key] = nil
                end
