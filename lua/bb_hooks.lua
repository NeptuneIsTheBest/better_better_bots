...
if RequiredScript == "lib/units/weapons/newnpcraycastweaponbase" then
    if Network:is_server() then
        Hooks:PostHook(NewNPCRaycastWeaponBase, "setup", "BB_NewNPCRaycastWeaponBase_setup_RemoveFriendlyMask",
...
if RequiredScript == "lib/units/weapons/npcraycastweaponbase" then
    if Network:is_server() then
        Hooks:PostHook(NPCRaycastWeaponBase, "setup", "BB_NPCRaycastWeaponBase_setup_RemoveFriendlyMask",
            remove_ai_and_players_from_bullet_mask)
    end
end
...
if RequiredScript == "lib/units/enemies/cop/actions/upper_body/copactionshoot" then
    if Network:is_server() then
        ...
        function CopActionShoot:_get_target_pos(shoot_from_pos, attention, ...)
            local target_pos, target_vec, target_dis, autotarget = _bb_orig_get_target_pos(self, shoot_from_pos,
                attention, ...)
        ...
        function CopActionShoot:_get_transition_target_pos(shoot_from_pos, attention, t, ...)
            local target_pos, target_vec, target_dis, autotarget = _bb_orig_get_transition_target_pos(self,
                shoot_from_pos, attention, t, ...)
...
if RequiredScript == "lib/units/enemies/cop/logics/coplogicbase" then
    if Network:is_server() then
        ...
        Hooks:PreHook(CopLogicBase, "_upd_attention_obj_detection", "BB_CopLogicBase_updAttentionObjDetection_FastDetect",
                        ...
                        if not att_tracker or chk_vis_func(my_tracker, att_tracker) then
                            ...
                            if att_handler and att_handler.get_attention and att_handler.get_detection_m_pos then
                                ...
                                if attention_pos then
                                    local vis_ray = World:raycast("ray", my_pos, attention_pos, "slot_mask", slotmask,
                                        "ray_type", "ai_vision")
                                    if not vis_ray or (vis_ray.unit and vis_ray.unit:key() == u_key) then
                                        local ok, att_obj = safe_call(
                                            CopLogicBase._create_detected_attention_object_data, t, unit, u_key,
                                            attention_info, settings)
                                        if not ok then att_obj = nil end

                                        if att_obj then
                                            local new_reaction = (settings and settings.reaction) or
                                                AIAttentionObject.REACT_IDLE
